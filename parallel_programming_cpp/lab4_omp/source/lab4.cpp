...
int sub_omp_atomic(int n, int* A, int* B, int* C) {
	int result = 1;
	int i, j, k;
...
int sub_omp_critical(int n, int* A, int* B, int* C) {
	int result = 1;
	int i, j, k;
...
void test_omp_atomic(int n, int* A, int* B, int* C) {
	double t1, t2, result;
	//Расчет произведения по данным условиям
	t1 = omp_get_wtime();
	result = sub_omp_atomic(n, A, B, C);
	t2 = omp_get_wtime();
	std::cout << "Тест c использованием atomic" << std::endl;
	std::cout << "Произведение: " << result << std::endl;
	std::cout << "Время исполнения основного вычислительного блока: " << t2 - t1 << std::endl;
}

void test_omp_critical(int n, int* A, int* B, int* C) {
	double t1, t2, result;
	//Расчет произведения по данным условиям
	t1 = omp_get_wtime();
	result = sub_omp_critical(n, A, B, C);
	t2 = omp_get_wtime();
	std::cout << "Тест c использованием critical" << std::endl;
	std::cout << "Произведение: " << result << std::endl;
	std::cout << "Время исполнения основного вычислительного блока: " << t2 - t1 << std::endl;
}
...
int main() {
	...
	while (true) {
		...
		test_omp_atomic(n, A, B, C);
		test_omp_critical(n, A, B, C);
	}
